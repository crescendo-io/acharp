...
$steps     = acharp_rows(get_field('heritage_steps'));
...
if (!$steps) {
    $steps = array(
        array(
            ...
            'texts'   => array(
                array('text' => 'C’est après la seconde guerre mondiale, en 1945, qu’Irène Charpentier crée l’Académie Charpentier. Le succès est très vite au rendez-vous et l’académie devient l’une des principales écoles d’arts appliqués en France.'),
                array('text' => 'En 1957, la famille Charpentier avait repris la direction de l’Académie de la Grande Chaumière, installée rue de la Grande-Chaumière à Montparnasse, un haut lieu de l’art, fréquenté par les plus grands noms de la peinture et de la sculpture du XX<sup>e</sup> siècle.'),
                array('text' => 'Au fil des décennies, l’Académie Charpentier a connu plusieurs évolutions majeures, toujours guidées par la même exigence artistique et le souci d’accompagner les transformations du monde créatif. D’abord dédiée à un large éventail de disciplines artistiques, dessin, peinture, sculpture, arts appliqués, l’école a progressivement affirmé son expertise dans le design d’espace et l’architecture intérieure, domaines où s’expriment pleinement sa rigueur, sa créativité et son ouverture d’esprit.'),
            ),
            'fallback' => array(
                'src' => $theme_uri . '/images/hero-atelier.jpg',
    ...
    <div class="heritage__inner">
        <div class="heritage__steps">
            <?php foreach ($steps as $index => $step) : ?>
                ...
                >
                    <h2 class="heritage__title"><?= acharp_inline_html($step['title'] ?? ''); ?></h2>

                    <?php foreach (acharp_rows($step['texts'] ?? array()) as $paragraph) : ?>
                        <?php $text = acharp_inline_html($paragraph['text'] ?? ''); ?>
                        <?php if ($text) : ?>
                            <p class="heritage__text"><?= $text; ?></p>
                        <?php endif; ?>
                    <?php endforeach; ?>
                </article>
            <?php endforeach; ?>
        ...
        <div class="heritage__media">
            <?php foreach ($steps as $index => $step) : ?>
                <figure
                    class="heritage__visual<?= 0 === $index ? ' is-active' : ''; ?><?= !empty($step['archive']) ? ' heritage__visual--archive' : ''; ?>"
                    data-heritage-visual="<?= (int) $index; ?>"
                >
                    <?= acharp_image_html($step['image'] ?? 0, 'large', $step['fallback'] ?? array()); ?>
                </figure>
            <?php endforeach; ?>
        </div>
    </div>
